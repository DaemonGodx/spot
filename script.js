...
async function getsongs(folder) {
    let a = await fetch(`images/songs/${encodeURIComponent(folder)}/`);
...
async function getfolders() {
    ...
    for (let i = 1; i < arr.length; i++) {
        ...
        let a = await fetch(`images/songs/${folderName}/info.json`);
        let response = await a.json();
        folderList.innerHTML += `<div class="playlist" data-folder="${folderName}">
                <img class="cover" src="images/songs/${folderName}/cover.jpg" alt="Playlist Cover">
